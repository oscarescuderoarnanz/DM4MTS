...
def loadData(i, norm, carpetas, dependencia):
    print("Balanced data --> " + dependencia)
    X_train = pd.read_csv('../data_generated_by_dtw/' + dependencia + '/' + carpetas[i] + '/X_train.csv')
    X_test = pd.read_csv('../data_generated_by_dtw/' + dependencia + '/' + carpetas[i] + '/X_test.csv')

    y_train = pd.read_csv('../../df_to_load/DataToPaperAndTFM_Mod1/Subconjuntos_3D/' + carpetas[i] + '/y_train_tensor.csv')
