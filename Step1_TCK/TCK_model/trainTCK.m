...
p.addParameter('minT', 1, @(z) assert(z>=1 && z<=T, 'The minimum length of time segments should be in [1,T]'));
p.addParameter('maxT', 2, @(z) assert(z>=1 && z<=T, 'The maximum length of time segments should be in [1,T]'));
p.addParameter('I', 20);
p.parse(varargin{:});
C = p.Results.C;
G = p.Results.G;
